...
export type NormalizedWords = { words: CompiledWord[]; audit: WordAudit };

/** Typed failure for note-level import validation (duplicate GUIDs, blank
 * fields, unusable pinyin/meaning/audio, unsafe distractor pools). */
export class WordImportError extends Data.TaggedError("WordImportError")<{
...
/** Normalizes every source note, dedupes semantically identical words, and
 * collects the quality audit. */
export const normalizeAndDedupeEffect = (
  notes: RawNote[],
  media: MediaIndex,
  overrides: Overrides,
): Effect.Effect<NormalizedWords, WordImportError | HanziError | SoundReferenceError, never> =>
  Effect.gen(function* () {
    ...
    for (const note of notes) {
      if (seenGuids.has(note.guid)) {
        return yield* Effect.fail(new WordImportError({ detail: `Duplicate Anki note GUID: ${note.guid}` }));
      }
      seenGuids.add(note.guid);
      const override = overrides[note.guid];
      const hanziInput = override?.displayHanzi ?? note.fields.hanzi;
      const hanzi = yield* normalizeHanziEffect(hanziInput);
      const sanitizedOriginal = sanitizeText(note.fields.hanzi);
      const nfkcOriginal = sanitizedOriginal.normalize("NFKC");
      if (override) {
        if (!override.reason.trim()) {
          return yield* Effect.fail(new WordImportError({ detail: `Override ${note.guid} has no review reason` }));
        }
      ...
      if (!displayPinyin) {
        return yield* Effect.fail(new WordImportError({ detail: `Note ${note.guid} has blank Pinyin` }));
      }
      if (!meaning) {
        return yield* Effect.fail(new WordImportError({ detail: `Note ${note.guid} has blank Meaning` }));
      }
      const acceptedPinyin = acceptedPinyinForms(displayPinyin);
      if (!acceptedPinyin.length) {
        return yield* Effect.fail(new WordImportError({ detail: `Note ${note.guid} has no canonical pinyin form` }));
      }
      ...
      if (!meaningKey) {
        return yield* Effect.fail(new WordImportError({ detail: `Note ${note.guid} has an empty normalized meaning key` }));
      }
      ...
      const audioFilename = yield* parseSoundReferenceEffect(note.fields.audioHanzi);
      if (!media.memberByFilename.has(audioFilename)) {
        return yield* Effect.fail(new WordImportError({
      ...
      if (priorFullId && priorFullId !== fullId) {
        return yield* Effect.fail(new WordImportError({ detail: `Stable word ID prefix collision: ${id}` }));
      }
...
export type MeaningIndexes = Pick<RuntimeDeck, "meaningIndex" | "meaningKeysByPartOfSpeech" | "allMeaningKeys"> & {
  minimumSafeDistractors: number;
};

/** Builds the sorted meaning indexes and validates that every word keeps at
 * least seven safe, non-colliding distractors. */
export const buildMeaningIndexesEffect = (words: RuntimeWord[]): Effect.Effect<MeaningIndexes, WordImportError, never> =>
  Effect.gen(function* () {
    ...
    for (const word of words) {
      ...
      if (safe.length < 7) {
        return yield* Effect.fail(new WordImportError({
          detail: `Word ${word.id} (${word.displayHanzi}) has only ${safe.length} safe meaning distractors`,
        }));
      }
      ...
      if (answerKeys.size === 0) {
        return yield* Effect.fail(new WordImportError({
          detail: `Word ${word.id} (${word.displayHanzi}) has a meaning with no A-Z hotkey: ${word.meaning}`,
        }));
      }
      const capacity = disjointShortcutCapacity(safe.map((key) => meaningIndex[key]!.label.trim()), answerKeys);
      if (capacity < 7) {
        return yield* Effect.fail(new WordImportError({
          detail: `Word ${word.id} (${word.displayHanzi}) has only ${capacity} distractors with non-colliding shortcuts`,
        }));
      }
    }
...
// the compiler consumes the Effect pipelines directly.

/** Synchronous boundary: throws the typed import error on failure. */
export function normalizeAndDedupe(notes: RawNote[], media: MediaIndex, overrides: Overrides): NormalizedWords {
  return Effect.runSync(normalizeAndDedupeEffect(notes, media, overrides));
}

/** Synchronous boundary: throws the typed `WordImportError` on failure. */
export function buildMeaningIndexes(words: RuntimeWord[]): MeaningIndexes {
  return Effect.runSync(buildMeaningIndexesEffect(words));
}
