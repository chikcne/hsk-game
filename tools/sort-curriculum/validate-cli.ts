...
if (manifest !== orderedJson(generated.manifest)) throw new Error("cards/curriculum.json is stale; run npm run sort:curriculum");
if (lock !== stableJson(generated.lock)) throw new Error("cards/curriculum.lock.json is stale; run npm run sort:curriculum");
console.log(`curriculum valid: ${Object.keys(generated.manifest).length} entries`);
