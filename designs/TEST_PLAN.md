...
- exact resume: each rating persists the next displayed card as `currentWordId`; JSON round-trip and grade relaunch preserve that pointer rather than recalculating it;
- review mode is unavailable below 20 `acquired_words` and draws only from that recency log, never from FSRS due dates: at 100+ words the deterministic base plan guarantees fixed quotas (ranks 0–19 twice, 20–99 once, Old fillers; boundaries 19/20/99/100; exact 200/500 configured counts), while 20–99-word pools proportionally scale tier boundaries, pressure, and base length; it interleaves tiers, and the pure spawn-session reducer enforces delayed (10 base spawns) additive retries until a clean correct encounter, never spawning a word concurrently, and completing only when all base spawns resolved, no enemy remains, and every obligation is cleared; battle outcomes never write word records;
